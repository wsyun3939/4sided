...
int bb2(Instance &instance, int UB_cur)
{
    ...
    for (auto element : src_vec)
    {
        if (element.block == 1)
        {
            ...
            {
            case Upp:
                src.x = instance_temp.config.pos[instance_temp.config.priority - 1].x;
                src.y = TIER - 1;
                // 積み替えるブロック位置を特定
                while (!instance_temp.config.block[src.x][src.y])
                {
                ...
                break;
            case Right:
                src.y = instance_temp.config.pos[instance_temp.config.priority - 1].y;
                src.x = STACK - 1;
                // 積み替えるブロック位置を特定
                while (!instance_temp.config.block[src.x][src.y])
                {
            ...
            for (dst.x = 0; dst.x < STACK; dst.x++)
            {
                if (!instance_temp.config.block[dst.x][TIER - 1])
                {
                    int LB_temp = 0;
                    while (!instance_temp.config.block[dst.x][dst.y - 1] && (dst.y > 0))
                        dst.y--;
            ...
            {
                if (!instance_temp.config.block[STACK - 1][dst.y])
                {
                    int LB_temp = 0;
                    while (!instance_temp.config.block[dst.x - 1][dst.y] && (dst.x > 0))
                        dst.x--;
                    if (src == dst)
                    {
                        dst.x = STACK - 1;
